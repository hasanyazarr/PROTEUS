...
% The banner carries only the solver policy, which is fixed for the run; the
% bubble count varies per frame and is reported on the frame line instead.
if useGPU
    run_log('banner', 'solver', ...
        ['MB solver: GPU-RK4 | batch %d | N_out=%d | ' ...
         'stride=%d, n_sub=%d, precision=%s, pressure=%s'], ...
        batchSize, M, batchSolverInfo{1}.stride, ...
        batchSolverInfo{1}.substeps, batchSolverInfo{1}.precision, ...
        batchSolverInfo{1}.pressureInterp);
else
    run_log('banner', 'solver', ...
        'MB solver: CPU ode45 | batch %d | N_out=%d', batchSize, M);
end
run_log('count', 'MB', N_MB);
