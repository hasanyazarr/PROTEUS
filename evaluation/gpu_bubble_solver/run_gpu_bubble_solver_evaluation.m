...
% The k-Wave capture is ~20 minutes of GPU time and depends on nothing the
% sweep varies - the phase step is swept over the replay, not the capture.
% Reuse it when a previous run already produced one in OUTPUTDIR.
addParameter(parser, 'ReuseCapture', false);
...
evaluationSettingsPath = fullfile(outputDir, 'evaluation_settings.mat');
reuseCapture = logical(parser.Results.ReuseCapture) && ...
    exist(capturePath, 'file') && exist(evaluationSettingsPath, 'file');
if reuseCapture
    fprintf(['Reusing the pressure capture at %s. The capture does not ' ...
        'depend on the phase step - that is swept over the replay.\n'], ...
        capturePath)
else
...
environment.reused_pressure_capture = reuseCapture;
